...
class RoutingRequestTransformer(Transformer):
    ...
    def transform(self, source: Source) -> Target:
        ...
        transformer = LaneWaypointTransformer(properties={"lanelet_map": lanelet_map, "projector": projector})

        if "apollo_map" in self.properties:
            transformer.properties["apollo_map"] = self.properties["apollo_map"]
        
        openscenario_waypoints = map(lambda lane_waypoint: (
            transformer.transform(source=lane_waypoint)
        ), source.waypoint)

        route_name = self.properties["route_name"] if "route_name" in self.properties else ""
        route = Route(closed=False, name=route_name, parameter_declarations=[], waypoints=openscenario_waypoints)
